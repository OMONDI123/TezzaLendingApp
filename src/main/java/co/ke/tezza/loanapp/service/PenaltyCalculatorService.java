...
public class PenaltyCalculatorService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PenaltyCalculatorService.class);
    ...
    /**
     * Check if main loan penalty should be calculated
     * UPDATED: Check if we're past the penalty start date (due date + grace period)
     */
    private boolean shouldCalculateMainLoanPenalty(MLoanApplication loan) {
        ...
        // Check if loan due date exists
        if (loan.getDueDate() == null) {
            return false;
        }
        
        // Get penalty start date (due date + grace period)
        Date penaltyStartDate = getPenaltyCalculationStartDate(loan, config);
        ...
        // Loan is eligible for penalty, check if installment penalties override
        boolean installmentOverride = config.isLoanOverDueChargePenaltyInstallmentDue();
        
        // If installmentOverride is TRUE, installment penalties override loan penalties
        // So we should NOT calculate main loan penalty
        return !installmentOverride;
    }

    /**
     * Consolidated logging method for penalty calculation summary
     */
    private void logPenaltySummary(MLoanApplication loan, MLoanProductConfiguration config,
        ...
        String summary = String.format("PENALTY_CHARGED | Loan: %d | Period: %s to %s | "
                + "Grace: %d days | Effective Days: %d | Config: PPSP=%s, PROOD=%s | "
                + "Principal: %s | Loan Bal: %s | "
                + "Penalty Base: %s | Base Amount: %s | Rate: %s%% | "
                + "Calculation: %s | Penalty: %s",
                ...
                config.isPeriodPaymentStopPenalty(),
                config.isPaymentReliefOnOverdueDebt(),
                principal.setScale(2, RoundingMode.HALF_UP),
                loanBalance.setScale(2, RoundingMode.HALF_UP),
                config.getPenaltyAppliesTo(), penaltyBaseAmount.setScale(2, RoundingMode.HALF_UP),
    ...
    /**
     * Calculate the base start date for penalty calculations
     * If lastPenaltyCalculationDate is null, use dueDate + gracePeriod
     */
    private Date getPenaltyCalculationStartDate(MLoanApplication loan, MLoanProductConfiguration config) {
    ...
    /**
     * Calculate the start date for current penalty calculation period
     * UPDATED: Improved logic to match installment service
     */
    private Date getCurrentPeriodStartDate(MLoanApplication loan, MLoanProductConfiguration config, Date calculationDate) {
        logger.debug("Calculating current period start date for loan ID: {}", loan.getLoanApplicationId());
        logger.debug("Next Penalty Date: {}, Last Penalty Date: {}", 
                loan.getNextPenaltyCalculationDate(), 
                loan.getLastPenaltyCalculationDate());
        ...
        // If we're already past the grace period end date,
        // we should start calculating from that date, not wait for next scheduled date
        if (calculationLocal.isAfter(penaltyStartLocal)) {
            // We're already past grace period - check if we have a next penalty date
            if (loan.getNextPenaltyCalculationDate() != null) {
                // Use the LATER of next penalty date or grace period end
                LocalDate nextCalcLocal = loan.getNextPenaltyCalculationDate().toInstant()
                        .atZone(ZoneId.systemDefault()).toLocalDate();
                LocalDate graceEndLocal = penaltyStartLocal;
                ...
                logger.debug("Past grace period - using max of nextPenaltyDate({}) and graceEnd({}) = {} as period start",
        ...
        if (loan.getLastPenaltyCalculationDate() != null) {
            Date frequencyBasedDate = calculateDateWithFrequency(loan.getLastPenaltyCalculationDate(), config, false);

            if (frequencyBasedDate != null) {
                logger.debug("Using last calculation date {} + frequency = {} as period start",
                        loan.getLastPenaltyCalculationDate(), frequencyBasedDate);
        ...
        logger.debug("Using default grace period end = {} as period start", penaltyStartDate);
     ...
     * @param forNextPeriod If true, calculate for next period; if false, calculate based on current period
    ...
    /**
     * Calculate next penalty calculation date
     * This should be: current calculation date + frequency period
     */
    ...
    /**
     * Calculate penalty for a loan application - MAIN LOAN PENALTY SERVICE
     * This service calculates penalties for loans when installment penalties don't override
     */
    public PenaltyCalculationResult calculatePenalty(MLoanApplication loan) {
        logger.debug("=== STARTING PENALTY CALCULATION FOR LOAN ID: {} ===", loan.getLoanApplicationId());

        if (loan == null || loan.getLoanProductConfiguration() == null) {
            logger.error("Invalid loan or loan product configuration is null");
            return new PenaltyCalculationResult(BigDecimal.ZERO, 0, "Invalid loan", null, null);
        }
        ...
        logger.debug("CURRENT LOAN STATE:");
        logger.debug("  - Due Date: {}", loan.getDueDate());
        logger.debug("  - Last Penalty Calculation Date: {}", loan.getLastPenaltyCalculationDate());
        logger.debug("  - Next Penalty Calculation Date: {}", loan.getNextPenaltyCalculationDate());
        logger.debug("  - Penalty Grace Period: {}", loan.getPenaltyGracePeriod());
        logger.debug("  - Config Grace Period: {}", config.getPenaltyGracePeriodDays());

        // Check if we should calculate main loan penalty
        if (!shouldCalculateMainLoanPenalty(loan)) {
            logger.debug("Skipping penalty calculation - installment penalties override OR loan not overdue");
            return new PenaltyCalculationResult(BigDecimal.ZERO, 0,
                    "No penalty - installment penalties override OR loan not overdue", 
                    loan.getNextPenaltyCalculationDate(), 
                    calculationDate);
        }
        ...
        Date dueDate = loan.getDueDate();
        if (dueDate == null) {
            logger.error("Loan due date is null");
            return new PenaltyCalculationResult(BigDecimal.ZERO, 0, "Loan due date is null", null, calculationDate);
        }

        // Get penalty calculation start date (dueDate + gracePeriod)
        Date penaltyStartDate = getPenaltyCalculationStartDate(loan, config);
        LocalDate penaltyStartLocal = penaltyStartDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

        // Check if we're past the penalty eligibility date
        if (calculationLocal.isBefore(penaltyStartLocal)) {
            logger.debug("Loan within grace period: {} < {}", calculationLocal, penaltyStartLocal);
            Date nextPenaltyDate = calculateNextPenaltyCalculationDate(calculationDate, config);
            return new PenaltyCalculationResult(BigDecimal.ZERO, 0, "No penalty - loan within grace",
                    nextPenaltyDate, calculationDate);
        }

        // Get current period start date based on last/next calculation dates
        Date currentPeriodStartDate = getCurrentPeriodStartDate(loan, config, calculationDate);
        LocalDate currentPeriodStartLocal = currentPeriodStartDate.toInstant()
                .atZone(ZoneId.systemDefault()).toLocalDate();

        // If calculation date is before current period start, no penalty yet for this period
        if (calculationLocal.isBefore(currentPeriodStartLocal)) {
            logger.debug("Before current period start: {} < {}", calculationLocal, currentPeriodStartLocal);
            Date nextPenaltyDate = calculateNextPenaltyCalculationDate(calculationDate, config);
            return new PenaltyCalculationResult(BigDecimal.ZERO, 0, "No penalty - before current period start",
                    nextPenaltyDate, calculationDate);
        }

        // Determine if we should use inclusive counting
        // FIXED: Match installment service logic - use inclusive counting when nextPenaltyCalculationDate is not null OR when at grace end
        boolean useInclusiveCounting = loan.getNextPenaltyCalculationDate() != null
                || calculationLocal.equals(penaltyStartLocal);
        logger.debug("Using inclusive counting: {} (nextPenaltyDate is {}null, at grace end: {})", 
                useInclusiveCounting, 
                loan.getNextPenaltyCalculationDate() == null ? "" : "not ",
                calculationLocal.equals(penaltyStartLocal));

        // Calculate effective days from current period start to calculation date
        int effectiveDaysOverdue = calculateEffectiveDaysOverdue(
                currentPeriodStartDate, calculationDate, config, loan, useInclusiveCounting);
        ...
        BigDecimal principal = loan.getApprovedAmount() != null ? loan.getApprovedAmount() : loan.getAppliedAmount();
        BigDecimal loanBalance = loan.getBalance();
        BigDecimal overdueAmount = getOverdueAmountForLoan(loan);
        ...
        PenaltyCalculationResult result = calculatePenaltyWithEffectiveDays(principal, outstandingBalanceToPass,
                overdueAmount, dueDate, calculationDate, loan.getLastPenaltyCalculationDate(), config,
                effectiveDaysOverdue);
        ...
        result = new PenaltyCalculationResult(result.getPenaltyAmount(), result.getDaysOverdue(),
                result.getMessage(), nextPenaltyDate, calculationDate);

        // Only log when penalty is actually charged
        if (result.getPenaltyAmount().compareTo(BigDecimal.ZERO) > 0) {
            logPenaltySummary(loan, config, currentPeriodStartDate, effectiveDaysOverdue,
        ...
        logger.debug("=== PENALTY CALCULATION COMPLETED ===");
        return result;
    }
    ...
    /**
     * Calculate effective days overdue based on payment exclusion rules
     * FIXED: Use correct overdue check for payment exclusion (due date only, not due date + grace)
     */
    public int calculateEffectiveDaysOverdue(Date startDate, Date endDate, MLoanProductConfiguration config,
        ...
        logger.debug("Loan ID: {}, Loan Due Date: {}", 
                loan.getLoanApplicationId(), 
        ...
        if (useInclusiveCounting) {
            ...
            // Use exclusive counting for dueDate + gracePeriod periods
        ...
        // FIXED: Use isLoanOverdueForPaymentExclusion() which checks against due date only
        // Not isLoanOverdue() which checks against due date + grace period
        boolean isLoanOverdue = isLoanOverdueForPaymentExclusion(loan, endLocal);
        
        // Detailed overdue check logging
        if (loan.getDueDate() != null) {
            LocalDate loanDueDate = loan.getDueDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            logger.debug("Loan Due Date: {}, Calculation Date: {}, Is Overdue for Payment Exclusion: {}", 
                    loanDueDate, endLocal, isLoanOverdue);
        }
        ...
        logger.debug("IsLoanOverdue (for payment exclusion): {}", isLoanOverdue);
        ...
        logger.debug("Found {} payment dates in period {} to {}", 
                paymentDates.size(), startLocal, endLocal);
        ...
        if (paymentReliefOnOverdueDebt) {
            ...
            return calculateDaysWithOverdueLogic(startLocal, endLocal, paymentDates, config, loan, isLoanOverdue, useInclusiveCounting);
        } else {
    ...
    /**
     * Check if loan is overdue at a given date FOR PAYMENT EXCLUSION
     * This checks against the due date (not due date + grace)
     */
    private boolean isLoanOverdueForPaymentExclusion(MLoanApplication loan, LocalDate calculationDate) {
        logger.debug("=== CHECKING IF LOAN IS OVERDUE FOR PAYMENT EXCLUSION ===");
        ...
        logger.debug("Loan is overdue for payment exclusion: {}", isOverdue);
        
        return isOverdue;
    }

    /**
     * Check if loan is overdue at a given date FOR PENALTY CALCULATION
     * Overdue means past the penalty start date (due date + grace period)
     */
    private boolean isLoanOverdue(MLoanApplication loan, LocalDate calculationDate) {
        logger.debug("=== CHECKING IF LOAN IS OVERDUE FOR PENALTY ===");
        ...
        // Get penalty start date (due date + grace period)
        Date penaltyStartDate = getPenaltyCalculationStartDate(loan, loan.getLoanProductConfiguration());
        LocalDate penaltyStartLocal = penaltyStartDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        
        boolean isOverdueForPenalty = calculationDate.isAfter(penaltyStartLocal);
        
        logger.debug("Penalty Start Date: {}, Calculation Date: {}", penaltyStartLocal, calculationDate);
        logger.debug("Is calculation date after penalty start date? {}", isOverdueForPenalty);
        logger.debug("Loan is overdue for penalty: {}", isOverdueForPenalty);
        
        return isOverdueForPenalty;
    }
    ...
    private int calculateDaysWithOverdueLogic(LocalDate startLocal, LocalDate endLocal, Set<LocalDate> paymentDates,
            MLoanProductConfiguration config, MLoanApplication loan, boolean isLoanOverdue, boolean useInclusiveCounting) {
        ...
        if (isLoanOverdue) {
            ...
            if (endLocal.isBefore(loanDueDate) || endLocal.isEqual(loanDueDate)) {
                ...
                return calculateDaysSplitPeriod(startLocal, endLocal, paymentDates, config, loanDueDate, useInclusiveCounting);
            }
        }
    }
    ...
    private int calculateDaysSplitPeriod(LocalDate startLocal, LocalDate endLocal, Set<LocalDate> paymentDates,
        ...
        if (!startLocal.isAfter(preDueEnd)) {
            preDueDays = calculateDaysExcludingPayments(startLocal, preDueEnd, paymentDates, config, useInclusiveCounting);
    ...
    private int calculateDaysExcludingPayments(LocalDate startLocal, LocalDate endLocal, Set<LocalDate> paymentDates,
        ...
        return calculateDaysExcludingPaymentsByFrequency(startLocal, endLocal, paymentDates, config, useInclusiveCounting);
    }

    /**
     * Calculate days excluding only specific payment days (for DAILY frequency)
     */
    private int calculateDaysExcludingPaymentDays(LocalDate startLocal, LocalDate endLocal,
     ...
     * FIXED: Added getNextPeriodStartDate() method and updated period transition logic
     */
    private int calculateDaysExcludingPaymentPeriods(LocalDate startLocal, LocalDate endLocal,
        ...
        while (!currentPeriodStart.isAfter(endLocal)) {
            ...
            // FIXED: Use getNextPeriodStartDate() to ensure consistent period transitions
     ...
     * ADDED: This method was missing from loan penalty service
     */
    ...
    /**
     * Calculate penalty amount using effective days - UPDATED TO MATCH INSTALLMENT SERVICE LOGIC
     */
    private BigDecimal calculatePenaltyAmountWithEffectiveDays(BigDecimal penaltyBaseAmount, int effectiveDaysOverdue,
        ...
        case PER_WEEK:
            // UPDATED: Match Installment Service logic - CEILING for first, FLOOR for subsequent
            int weeksOverdue;
            if (lastPenaltyCalculationDate == null) {
                ...
                logger.debug("PER_WEEK (FIRST): {} days = {} weeks (CEILING)", 
        ...
        case PER_MONTH:
            ...
            if (lastPenaltyCalculationDate == null) {
                ...
                logger.debug("PER_MONTH (FIRST): {} days = {} months (CEILING)", 
        ...
        case PER_CYCLE:
            ...
            if (lastPenaltyCalculationDate == null) {
                ...
                logger.debug("PER_CYCLE (FIRST): {} days = {} cycles of {} days (CEILING)", 
    ...
    /**
     * Get overdue amount for loan
     */
    public BigDecimal getOverdueAmountForLoan(MLoanApplication loan) {
        if (loan.getDueDate() == null || loan.getDueDate().after(new Date())) {
            return BigDecimal.ZERO;
        }

        if (!loan.isHasInstallments()) {
            return loan.getBalance() != null ? loan.getBalance() : BigDecimal.ZERO;
        }
    ...
    /**
     * NEW: Special logic for DAILY frequency with payment relief - RESETS after each payment
     */
    ...
    private int calculateDaysExcludingPaymentsByFrequency(LocalDate startLocal, LocalDate endLocal, 
        ...
        switch (calculationBase) {
            case PER_DAY:
                // DAILY: Use new reset logic for payment relief
            ...
            case PER_CYCLE:
                ...
                return calculateDaysExcludingPaymentPeriods(startLocal, endLocal, paymentDates, config, useInclusiveCounting);
            ...
            default:
                // Fallback to daily calculation with reset logic
                return calculateDaysExcludingPaymentDaysWithReset(startLocal, endLocal, paymentDates, useInclusiveCounting);
        }
    }
